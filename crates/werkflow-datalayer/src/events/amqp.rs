...
pub struct MessageConsumer<T> where for <'de> T : Serialize + Deserialize<'de> + Clone {
    shutdown_channel: Sender<()>,   
    message_channel: Receiver<T>, 
    _pd : PhantomData<T>
}
pub struct MessageQueue<T> where for <'de> T : Serialize + Deserialize<'de> + Clone {       
    channel : Arc<RwLock<Option<Channel>>>,
    connection : Arc<RwLock<Option<Connection>>>,
    _pd : PhantomData<T>
}

impl <T> MessageConsumer<T> where for <'de> T : Serialize + Deserialize<'de> + Clone {
    /// Blocking method that will receive message when one is available in the queue
    pub async fn recv(&self) -> Result<T> {
        self.message_channel.recv()
...
impl <'a, T> MessageQueue<T>  where for <'de> T : Serialize + Deserialize<'de> + Clone + Send + 'static {
    ...
    async fn publish(&self, msg : Message<T>) -> Result<MessageResult> {       
        ...
        let publish_result= self.channel.read().await.as_ref().map(|channel| {
            let exchange = Exchange::direct(channel);
            match msg {
                Message::AsBytes(routing_key, msg) => {
                    let properties = AmqpProperties::default().with_content_type("application/octet-stream".into());
                    exchange
                            .publish(Publish::with_properties(&bincode::serialize(&msg)?, routing_key, properties))        
                }
                Message::AsJson(routing_key, obj) => {
                    exchange
                        .publish(Publish::new(serde_json::to_string(&obj)
                            .map_err(|err| anyhow!("Could not map object to json: {}", err))?.as_bytes(), routing_key))
                }
    ...
    async fn consumer<'b : 'a,FN>(&'a self, queue_name: &'static str, processor: FN ) -> MessageConsumer<T>
        where 
            FN: Fn(T) + Send + 'static {
        // Open a channel - None says let the library choose the channel ID.        
        ...
        let chan = c_w.as_mut().unwrap().open_channel(None)
            .map_err(|err| anyhow!("Could not open channel: {}", err)).unwrap();
        ...
        let jh = tokio::spawn(async move {
            ...
            let queue: Queue = chan.queue_declare(queue_name, QueueDeclareOptions::default())
                .map_err(|err| anyhow!("Could not create or declare queue: {}", err)).unwrap();
                    ...
                    ConsumerMessage::Delivery(delivery) => {
                        ...
                        let result:T = if let Some(content_type) = props.content_type() {
                            match content_type.as_str() {
                                "application/octet-stream" => bincode::deserialize_from(&dev[..]).unwrap(),
                                _ => {
    ...
    async fn close(&self) -> Result<()>{            
        ...
        if let Some(conn) = opt.take() {        
            conn.close().map_err(|err| anyhow!("Could not close connection: {}", err))
        } else {
            Ok(())
        }
    }
...
mod test {
    ...
    async fn test_amqp () -> Result<()> {
        ...
        let recv = mp.consumer("test_queue", |msg| {
