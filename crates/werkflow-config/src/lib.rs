...
pub async fn read_config<T>(source : ConfigSource) -> Result<T, anyhow::Error> where for <'de> T : Deserialize<'de> {

    match source {
        ConfigSource::String(file) => {
            let mut config = Config::new();

            config.merge(File::from_str(&file, FileFormat::Toml))
                .map_err(|err| anyhow!("Could get config rom string provided. {}", err))?;
            
            config.try_into::<T>()
                .map_err(|err| anyhow!("Could not coerce configuration into DataSourceConfig: {}", err))
        }
        ConfigSource::Http(url) => { 
            read_web_config::<T>(url.clone()).await
...
async fn read_web_config<T>(action : HttpAction) -> Result<T, anyhow::Error> where for <'de> T : Deserialize<'de> {
    ...
    let src = final_action
            .send()
            .await.map_err(|err| anyhow!("Error contacting url {:?}, {}", action, err)).unwrap()
            .text()
    ...
    config.merge(File::from_str(&src, FileFormat::Toml))
        .map_err(|err| anyhow!("Could not create config from url {:?}: {}", action, err))?;

    config.try_into::<T>()
        .map_err(|err| anyhow!("Could not coerce into a config object {:?}: {}", action, err))
}
...
use super::*;
    ...
    async fn test_read_config() -> Result<(), anyhow::Error> {
        ...
        let cfg = read_config::<DataSourceConfig>(ConfigSource::File(filename.into()))
        .await?;
        ...
        let config = warp::path!("config")
            .map(move || format!("{}",config_file));

        tokio::spawn(warp::serve(config)
            .run(([127, 0, 0, 1], 5042)));

        let config_web_result = read_web_config::<DataSourceConfig>(HttpAction::Get("http://localhost:5042/config".into()))
            .await
