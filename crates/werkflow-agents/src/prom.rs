...
lazy_static! {
    ...
    pub static ref WORKLOAD_TIME_COLLECTOR: HistogramVec = HistogramVec::new(
        HistogramOpts::new("processing_time", "Processing Time"),
        &["env"]
    )
    .expect("metric can be created");
}
...
pub fn register_custom_metrics() {
    ...
    REGISTRY
        ...
        .expect("collector can be registered");
    REGISTRY
        .register(Box::new(WORKLOAD_TOTAL_TIME.clone()))
        .expect("collector can be registered");
    ...
    REGISTRY
        .register(Box::new(WORKLOAD_TIME_COLLECTOR.clone()))
        .expect("collector can be registered");
