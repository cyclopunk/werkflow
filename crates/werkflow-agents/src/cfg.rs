...
pub trait ConfigDefinition {
    fn from_file<T>(path : &Path) -> Result<T> where for <'d> T : ConfigDefinition + Deserialize<'d>, Self: Sized {
        werkflow_config::blocking::from_file(path)
    }
    fn get<T>(&self, key : &str) -> T where for <'d> T: Deserialize<'d>, Self: Serialize {
        self.try_get(key).unwrap()
    }
    fn try_get<T>(&self, key :  &str) -> Result<T> where for <'d> T: Deserialize<'d>, Self: Serialize {
    ...
    fn merge<'d, T>(config : impl Serialize) -> Result<T> where for<'a> T: Deserialize<'a> {
...
#[test] 
fn test_config_get() {
    let dns = DnsConfiguration { api_key: "test".to_string() };
